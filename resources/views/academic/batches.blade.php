                ...
                @if (Auth::user()->hasAnyRole(['Admin']))
                    <div class="text-right col-auto">
                        <a type="button" class="btn btn-sm btn-outline-primary shadow-sm"
                            href="{{ route('batches.create') }}">New</a>
                    </div>
                @endif
            </div>
        </div>
                        ...
                        @foreach ($batches as $batch)
                            <tr data-bid="{{ $batch->id }}">
                                ...
                                <td><span data-toggle="tooltip" data-placement="top"
                                        title="{{ $batch->academic_year->status }}"
                                        class="{{ $batch->academic_year->status == 'Active' ? 'text-primary' : ($batch->academic_year->status == 'Planning' ? 'text-dark' : 'text-secondary') }}"><i
                                            class="fas fa-check-circle"></i></span> {{ $batch->academic_year->name }}</td>
                                <td>
                                        ...
                                        <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                            <a class="dropdown-item"
                                                href="{{ route('kafa.exams.results.batch', ['id' => $batch->id]) }}"><i
                                                    class="fas fa-graduation-cap"></i> KAFA Results</a>
                                            <a class="dropdown-item"
                                                href="{{ route('students.batch', ['id' => $batch->id]) }}"><i
                                                    class="fas fa-user-graduate"></i> Students</a>
                                            @if (Auth::user()->hasAnyRole(['Admin']))
                                                <div class="dropdown-divider"></div>
                                                <a class="dropdown-item "
                                                    href="{{ route('batches.edit', ['id' => $batch->id]) }}"><i
                                                        class="far fa-edit"></i> Edit</a>
                                                <a class="dropdown-item text-danger"
                                                    href="{{ route('batches.delete', ['id' => $batch->id]) }}"><i
