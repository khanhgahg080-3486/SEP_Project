@extends('layouts.pdf')
@section('header')
    <p class="text-center"><b>My Kafa Academic Transcript</b></p>
...
@section('content')
        ...
        <thead class="bg-primary text-light">
            <tr>
                ...
                @foreach ($exams as $exam)
                    <th scope="col" class="text-center"><small>{{ $exam->module_code }}<span>
                                ({{ $exam->exam_type }})</span></small></th>
                @endforeach
            </tr>
        </thead>
        <tbody>
            @foreach ($results as $index => $result_row)
                <tr class="border-bottom border-info">
                    ...
                    <td class="text-center">
                        @if ($students[$index]->kafa_exam_pass == $students[$index]->kafa_exam_modules)
                            <small>Pass</small>
                        @else
    ...
    <p>
        @foreach ($exams as $exam)
            <small><b class="text-dark">{{ $exam->module_code }}</b> - {{ $exam->module_name }}
                ({{ $exam_types[$exam->exam_type] }})</small>
        @endforeach
    </p>
@endsection
